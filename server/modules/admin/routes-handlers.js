...
module.exports = ({ gstore }, { blog }) => {
  ...
  const dashboard = async (req, res) => {
    ...
    res.render(template, {
      ...
      pageId: 'admin-index',
      protectedBlogPosts: protectedBlogPosts ? protectedBlogPosts.join(',') : '',
    });
  };

  const createPost = async (req, res) => {
    const template = path.join(templatesDir, 'edit');
